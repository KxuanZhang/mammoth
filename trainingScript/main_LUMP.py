...
def main(device, args):
    ...
    for t in range(dataset.N_TASKS):
        ...
        for epoch in global_progress:
            ...
            local_progress = tqdm(train_loaders[t], desc=f'Epoch {epoch}/{args.train.num_epochs}',
                ...
                if args.train.knn_monitor:
                    ...
                    for i in eval_tids:
                        acc, acc_mask = knn_monitor(model.net.module.backbone, dataset, dataset.memory_loaders[i],
                                                    dataset.test_loaders[i],
                                                    device, args.cl_default, task_id=i,
                                                    k=min(args.train.knn_k, len(eval_tids)))
